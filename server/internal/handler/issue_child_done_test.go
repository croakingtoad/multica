...
func TestChildAttentionCustomInReviewWakesParentAgent(t *testing.T) {
	const customStatus = "child_attention_review"
	createTestCustomStatus(t, customStatus, "in_review")
	fx := newChildDoneFixture(t, "in_progress")
	...
	updateChildStatus(t, fx.child.ID, customStatus)

	content := parentSystemCommentContent(t, fx.parent.ID)
	if !strings.Contains(content, "is in review") {
		t.Errorf("expected custom in-review status to use canonical handoff label, got: %s", content)
	}
	if got := countPendingTasksForAgent(t, fx.parent.ID, agentID); got != 1 {
		t.Fatalf("expected 1 pending parent task for custom in-review child, got %d", got)
	}
}
